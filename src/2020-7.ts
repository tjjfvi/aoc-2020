...
function getSize(bag = "shiny gold bag"): number{
  console.log(bag)
  const line = rules.find(x => x.startsWith(bag))!;
  const contains = line.split("contain ")[1];
  if(contains === "no other bags.")
    return 1;
  return 1 + contains.slice(0, -1).split(", ").map(x => +x.split(" ")[0] * getSize(x.split(" ").slice(1).join(" "))).reduce((a, b) => a + b, 0);
}
