...
console.log(
  input.split("\n\n").length,
  // input.split("\n\n")
  // .filter(str => fields.every(f => str.includes(f + ":")))
  // .map(str => str.replace(/ /g, "\n"))
  // .map(str => Object.assign({}, ...str.split("\n").map(p=>p.split(":")).map(([k,v])=>({ [k]: v }))) as (
  //   Record<typeof fields[number], string>
  // ))
  // .map(obj => [
  //   +obj.byr >= 1920, +obj.byr <= 2002,
  //   +obj.iyr >= 2010, +obj.iyr <= 2020,
  //   +obj.eyr >= 2020, +obj.eyr <= 2030,
  //   obj.hgt.slice(-2) === "cm"
  //     ? +obj.hgt.slice(0, -2) >= 150 && +obj.hgt.slice(0, -2) <= 193
  //     : +obj.hgt.slice(0, -2) >= 59 && +obj.hgt.slice(0, -2) <= 76,
  //     ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"].includes(obj.ecl)
  // ].every(x => x))
  input.split("\n\n").filter(str => fields.every(f => str.includes(f + ":"))).filter(str =>[
    /[^]*byr:(19[23456789]\d|200[012])\b/.test(str),
    /[^]*iyr:(201\d|2020)\b/.test(str),
    /[^]*eyr:(202\d|2030)\b/.test(str),
    /[^]*hgt:(1[5678]\dcm|19[0123]cm|59in|6\din|7[0123456]in)\b/.test(str),
    /[^]*hcl:(#[0123456789abcdef]{6})\b/.test(str),
    /[^]*ecl:(amb|blu|brn|gry|grn|hzl|oth)\b/.test(str),
    /[^]*pid:(\d{9})\b/.test(str),
  ].every(x => x)).length
)
